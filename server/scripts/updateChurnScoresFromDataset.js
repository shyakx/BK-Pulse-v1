/**
 * Update Churn Scores and Risk Levels from Dataset
 * Reads the dataset and updates existing customers with churn scores
 */
...
async function updateChurnScores() {
  ...
  try {
    ...
    // Create a map of customer_id -> churn data
    const churnMap = new Map();
    records.forEach(row => {
      const customerId = String(row.Customer_ID || row.customer_id);
      const churnProbability = parseFloat(row.Churn_Probability || 0);
      if (churnProbability > 0) {
        const churnScore = Math.round(churnProbability * 100 * 10) / 10;
        const riskLevel = calculateRiskLevel(churnProbability);
        churnMap.set(customerId, { churnScore, riskLevel });
      }
    });
    
    console.log(`📊 Found ${churnMap.size} customers with churn data\n`);
    ...
    for (let i = 0; i < dbCustomers.length; i += BATCH_SIZE) {
      ...
      for (const customerId of batch) {
        const churnData = churnMap.get(customerId);
        if (churnData) {
          updates.push({
            customerId,
            churnScore: churnData.churnScore,
            riskLevel: churnData.riskLevel
          });
        } else {
          notFound++;
        }
      ...
      if (updates.length > 0) {
        ...
        const caseChurnScore = updates.map((u, idx) => 
          `WHEN '${u.customerId}' THEN ${u.churnScore}`
        ).join(' ');
        const caseRiskLevel = updates.map((u, idx) => 
          `WHEN '${u.customerId}' THEN '${u.riskLevel}'`
        ).join(' ');
        
        const updateQuery = `
          UPDATE customers 
          SET 
            churn_score = CASE customer_id ${caseChurnScore} END,
            risk_level = CASE customer_id ${caseRiskLevel} END,
    ...
    const verifyResult = await pool.query(`
      SELECT 
        COUNT(*) as total,
        COUNT(churn_score) as with_churn_score,
...
module.exports = { updateChurnScores };
